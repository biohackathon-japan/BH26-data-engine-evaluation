...
Once the subject matter confirms that the agent has an accurate representation of the data structure and how the target data fits into that structure, the agent begins to draft CONSTRUCT queries. CONSTRUCT queries do not require an additional step of triple reconstruction across tables, and ensure fidelity between target triples and the extracted triples. Those queries are in a multi-step decision process:

**Step One: verify schema with an instance query** The expert's own instance level diagram, built around one concrete worked example, is checked edge by edge against the live endpoint before a single query is written. Every edge it shows is either confirmed real and present exactly as drawn, or the discrepancy is surfaced immediately, not smoothed over.

**Step Two: decide what the filter actually filters.** A target criteria can restrict which entries appear at all, or it can restrict which of an already included entry's own sub parts get expanded. These are different decisions, and a query has to be deliberate about which one it is making. 

An earlier draft of that same query got this wrong in a way only visible by checking actual output, not by reading the query text. It constructed a disease's `sio:SIO_000255` edges unconditionally but only expanded the human tagged blank nodes, leaving the mouse tagged ones referenced in the output with no triples of their own, a dangling reference. It was caught by inspecting the worked example's own raw output before finalizing the query.

**Step Three: split a query that runs too close to the timeout.** Performance is checked by agent while a query is still being drafted, and splits into separate queries if necessary. The split is kept permanently in the document once drafted. This is out of abundance of caution and is a decision point to address if significant changes are made to the database that make the split unnecessary, or requires a split along a different retrival pattern. 

**Step Four, keep the same pointer discipline as the pipeline.** Object URIs that were previously identified as the end of an extraction patter, aka a "pointer URI" are confirmed and the restriction discipline that was established in that dataset's own `tohsa_step3_predicate_discovery.md` in maintained.

**Step Five, run a deep cross check before sending it out.** This is the step that sets this kind of document apart from an internal working note, and it runs specifically because the document is about to leave this project for an external domain expert's own independent review. A full adversarial pass is run after the rest of the document otherwise looks complete, every check live against the endpoint, not inferred from the query text.

**Step Six: corrections an amendments based on expert review** A second round of expert review can show an earlier query was incomplete. A specific numeric disagreement raised by the expert is resolved by reproducing the expert's own query, not merely asserting the agent-queried number is right. If target triple coverage is insufficient or incorrect, the ammendment is noted in maintained in the prose docuemnts, but the incorrect query is overwritten. For example, the first draft of queries for the Disease dataset contained a "Query C" that restricted a phenotype bridge to cross references containing the literal string "OMIM". A second expert diagram showed this coverage is incomplete, and the corrected version was replaced with a new Query D, with a short note explaining why, since keeping both risks a future reader running the narrower, incomplete one by mistake.

**Step Seven: cite every number the same way as everywhere else.** Every SPARQL query, every live count, and every worked example in `query.md` is linked to its own numbered entry in `refDocs/query.md`, the same `[[N]](../refDocs/query.md#N-slug)` convention already used for `tohsa_step1_extraction_criteria.md` through `tohsa_step3_fetch_summary.md`, with its own independent numbering, not continuing the step documents' own sequence.
